...
function extractOTP(text) {
  ...
  const patterns = [
    /(?:code|kode|otp|pin|verifikasi|verification|sandi|password|masuk|akses sementara)\s*[:=]?\s*[\s]*?(\d{4,8})/gi,
  ...
  // Also try on raw text for HTML-embedded codes
  const htmlCodePattern = />(\d{4,8})</g;
  let m;
  while ((m = htmlCodePattern.exec(text)) !== null) {
    codes.add(m[1]);
  }
...
function extractLinks(text) {
  ...
  // Dedupe and filter out tracking/unsubscribe junk
  const seen = new Set();
...
async function grabOTP(config, query = "subject:OTP OR subject:verification OR subject:code newer_than:1d") {
  ...
  return emails.map((email) => {
    ...
    const links = extractLinks(email.body || email.snippet);
...
export { getGmailClient, searchEmails, extractOTP, extractLinks, extractBody, grabOTP };
