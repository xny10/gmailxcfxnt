...
function extractButtonLinks(text) {
  const buttonKeywords = /dapatkan kode|get code|sign in|masuk|verify|verifikasi|konfirmasi|confirm|login|akses|access|reset|update/i;
  const results = [];
  const seen = new Set();

  // Match <a> tags and check if their text matches button keywords
  const anchorPattern = /<a\s[^>]*href\s*=\s*["']([^"']+)["'][^>]*>(.*?)<\/a>/gis;
  let m;
  while ((m = anchorPattern.exec(text)) !== null) {
    const url = m[1].replace(/&amp;/g, "&");
    const linkText = m[2].replace(/<[^>]+>/g, "").trim();
    if (buttonKeywords.test(linkText) && !seen.has(url)) {
      seen.add(url);
      results.push({ url, label: linkText });
    }
  }
